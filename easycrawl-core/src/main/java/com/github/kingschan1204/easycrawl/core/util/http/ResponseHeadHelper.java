 ...
 * http响应头工具类
 *
 * @author kings.chan 2023-4-21
 */
@Slf4j
public class ResponseHeadHelper {
  /**
   * HTTP响应中发送的数据如何被显示或保存。它通常用于控制浏览器如何处理HTTP响应的附件，以及在下载文件时提示用户文件的名称和类型等信息。 Content-disposition:
    ...
    try {
      String key =
          responseHeaders.keySet().stream()
              .filter(r -> r.matches("(?i)Content-disposition"))
              .findFirst()
              .get();
  ...
  public boolean fileContent() {
    return responseHeaders.keySet().stream()
        ...
        .anyMatch(r -> r.equals(CONTENT_DISPOSITION.toLowerCase()));
  }
}

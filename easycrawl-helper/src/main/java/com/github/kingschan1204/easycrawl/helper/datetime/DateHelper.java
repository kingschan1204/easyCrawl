...
public class DateHelper {
  ...
  public static DateHelper of(String text) {
    ...
    if (date.matches("\\d{4}-\\d{1,2}-\\d{1,2}")) {
      ...
      date =
          String.format(
              "%s-%02d-%02d 00:00:00",
              array[0], Integer.valueOf(array[1]), Integer.valueOf(array[2]));
    } else if (date.matches("\\d{4}-\\d{1,2}-\\d{1,2} \\d{2}:\\d{2}:\\d{2}")) {
      String[] array = date.replaceAll("\\s.*", "").split("-");
      date =
          String.format(
              "%s-%02d-%02d %s",
              array[0], Integer.valueOf(array[1]), Integer.valueOf(array[2]), date.split("\\s")[1]);
    } else if (date.matches("\\d{8}")) {
      date =
          String.format(
              "%s-%s-%s 00:00:00", date.substring(0, 4), date.substring(4, 6), date.substring(6));
    } else {
  ...
  public Integer dateInt() {
    return Integer.valueOf(format("yyyyMMdd"));
  }
   ...
   * @return
   */
  public long getMillisDiff(LocalDateTime dateTime) {
    return Duration.between(getLocalDateTime(), dateTime).toMillis();
  }
   ...
   * @return
   */
  public static Duration getMillisDiff(LocalDateTime startTime, LocalDateTime endTime) {
